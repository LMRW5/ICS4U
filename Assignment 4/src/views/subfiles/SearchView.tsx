...
import type { SearchProps } from "@/core/types";
...
export function Searchview() {
  ...
  const tmdbData = useTmdb<SearchProps>(`https://api.themoviedb.org/3/search/${type}`, { query: debouncedQuery, page: page }, [
    debouncedQuery,
    type,
    page,
  ]).data;
  if (!tmdbData) {
    return <h1 className="text-3xl font-bold mb-6">no results found</h1>;
  }
  ...
  return (
    <section className="max-w-[1200px] mx-auto p-5 space-y-3">
      <h1 className="text-3xl font-bold mb-6">Search for: {query}</h1>
